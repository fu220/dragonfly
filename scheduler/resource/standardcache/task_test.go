...
var (
	...
	mockTaskID                         = idgen.CacheTaskIDV2ByURLBased(mockTaskURL, &mockTaskPieceLength, mockTaskTag, mockTaskApplication, mockTaskFilteredQueryParams)
...
func TestTask_NewTask(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expect: func(t *testing.T, task *Task) {
				...
				assert.Equal(task.Type, commonv2.TaskType_CACHE)
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			tc.expect(t, NewTask(mockTaskID, mockTaskURL, mockTaskTag, mockTaskApplication, commonv2.TaskType_CACHE, mockTaskFilteredQueryParams, mockTaskHeader, mockTaskBackToSourceLimit, tc.options...))
		})
	}
}
...
func TestTask_LoadPeer(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			task := NewTask(mockTaskID, mockTaskURL, mockTaskTag, mockTaskApplication, commonv2.TaskType_CACHE, mockTaskFilteredQueryParams, mockTaskHeader, mockTaskBackToSourceLimit)
...
func TestTask_CanBackToSource(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			task := NewTask(mockTaskID, mockTaskURL, mockTaskTag, mockTaskApplication, commonv2.TaskType_CACHE, mockTaskFilteredQueryParams, mockTaskHeader, tc.backToSourceLimit)
			tc.run(t, task)
		})
	}
}
