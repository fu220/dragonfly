...
func TestPeer_NewPeer(t *testing.T) {
	...
	defer ctl.Finish()
	stream := schedulerv2mock.NewMockScheduler_AnnounceCachePeerServer(ctl)
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			mockTask := NewTask(mockTaskID, mockTaskURL, mockTaskTag, mockTaskApplication, commonv2.TaskType_CACHE, mockTaskFilteredQueryParams, mockTaskHeader, mockTaskBackToSourceLimit, WithDigest(mockTaskDigest))
...
func TestPeer_LoadAnnouncePeerStream(t *testing.T) {
	tests := []struct {
		name   string
		expect func(t *testing.T, peer *Peer, stream schedulerv2.Scheduler_AnnounceCachePeerServer)
	}{
		{
			name: "load stream",
			expect: func(t *testing.T, peer *Peer, stream schedulerv2.Scheduler_AnnounceCachePeerServer) {
