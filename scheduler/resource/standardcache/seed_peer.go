...
//go:generate mockgen -destination seed_peer_mock.go -source seed_peer.go -package standardcache
...
type SeedPeer interface {
	...
	TriggerDownloadTask(context.Context, string, *dfdaemonv2.DownloadCacheTaskRequest) error
...
// TriggerDownloadTask triggers the seed peer to download task.
// Used only in v2 version of the grpc.
func (s *seedPeer) TriggerDownloadTask(ctx context.Context, taskID string, req *dfdaemonv2.DownloadCacheTaskRequest) error {
	...
	stream, err := client.DownloadCacheTask(ctx, taskID, req)
	if err != nil {
		return err
	}
