...
func WithAnnouncePeerStream(stream schedulerv2.Scheduler_AnnounceCachePeerServer) PeerOption {
	return func(p *Peer) {
...
// Used only in v2 version of the grpc.
func (p *Peer) LoadAnnouncePeerStream() (schedulerv2.Scheduler_AnnounceCachePeerServer, bool) {
	rawStream := p.AnnouncePeerStream.Load()
	if rawStream == nil {
		return nil, false
	}

	return rawStream.(schedulerv2.Scheduler_AnnounceCachePeerServer), true
}

// StoreAnnouncePeerStream set the grpc stream of Scheduler_AnnouncePeerServer,
// Used only in v2 version of the grpc.
func (p *Peer) StoreAnnouncePeerStream(stream schedulerv2.Scheduler_AnnounceCachePeerServer) {
