...
// @Summary Create Cache Peer
// @Description Create by json config
// @Tags Peer
// @Accept json
// @Produce json
// @Param Peer body types.CreatePeerRequest true "Peer"
// @Success 200 {object} models.CachePeer
// @Failure 400
// @Failure 404
// @Failure 500
// @Router /api/v1/cache-peers [post]
func (h *Handlers) CreateCachePeer(ctx *gin.Context) {
	var json types.CreatePeerRequest
	if err := ctx.ShouldBindJSON(&json); err != nil {
		ctx.JSON(http.StatusUnprocessableEntity, gin.H{"errors": err.Error()})
		return
	...
	peer, err := h.service.CreateCachePeer(ctx.Request.Context(), json)
	if err != nil {
		ctx.Error(err) // nolint: errcheck
		return
	}
...
// @Failure 404
// @Failure 500
// @Router /api/v1/cache-peers/{id} [delete]
func (h *Handlers) DestroyCachePeer(ctx *gin.Context) {
	var params types.PeerParams
	if err := ctx.ShouldBindUri(&params); err != nil {
		ctx.JSON(http.StatusUnprocessableEntity, gin.H{"errors": err.Error()})
		return
	...
	if err := h.service.DestroyCachePeer(ctx.Request.Context(), params.ID); err != nil {
		ctx.Error(err) // nolint: errcheck
		return
	}
...
// @Failure 404
// @Failure 500
// @Router /api/v1/cache-peers/{id} [get]
func (h *Handlers) GetCachePeer(ctx *gin.Context) {
	var params types.PeerParams
	if err := ctx.ShouldBindUri(&params); err != nil {
		ctx.JSON(http.StatusUnprocessableEntity, gin.H{"errors": err.Error()})
		return
	...
	peer, err := h.service.GetCachePeer(ctx.Request.Context(), params.ID)
	if err != nil {
		ctx.Error(err) // nolint: errcheck
		return
	}
...
// @Summary Get Cache Peers
// @Description Get Cache Peers
// @Tags Peer
// @Accept json
// @Produce json
// @Param page query int true "current page" default(0)
...
// @Failure 404
// @Failure 500
// @Router /api/v1/cache-peers [get]
func (h *Handlers) GetCachePeers(ctx *gin.Context) {
	var query types.GetPeersQuery
	if err := ctx.ShouldBindQuery(&query); err != nil {
		ctx.JSON(http.StatusUnprocessableEntity, gin.H{"errors": err.Error()})
		return
	...
	h.setPaginationDefault(&query.Page, &query.PerPage)
	peers, count, err := h.service.GetCachePeers(ctx.Request.Context(), query)
	if err != nil {
		ctx.Error(err) // nolint: errcheck
		return
	}
